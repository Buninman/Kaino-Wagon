...
void main()
{
	...
	// Perlin noise
	// float strenght = cnoise(vec3(vUv * 5.0, uTime * 0.0004 * 0.5));
	float strenght = cnoise(vec3(vUv * 5.0, uTime * uTimeFrequency));
	...
	// Final colors
	vec3 colorFire = mix(uColorDown, uColorUp, strenght);
	vec3 bakedNeutral = texture2D(uBakedNeutralTexture, vUv).rgb;

	vec3 mixColor = mix(bakedNeutral, colorFire, uNeutralMix);
